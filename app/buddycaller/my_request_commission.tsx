    ...
    const fetchRows = React.useCallback(async (opts?: { silent?: boolean }) => {
        ...
        const exec = (async () => {
            ...
            try {
                ...
                const runnerIds = Array.from(new Set(commissions.map((r) => r.runner_id).filter(Boolean) as string[]));
...
/* MAIN */
export default function MyCommissionRequestsMobile() {
    const router = useRouter();
    const { loading, firstName } = useAuthProfile();
    const insets = useSafeAreaInsets();
    const { initialLoading, rows: commissions, runnerNameMap, refetch } = useMyCommissions();
...
function CommissionCardMobile({ commission }: { commission: Commission }) {
    ...
    const handlePress = () => {
        if (commission.status === "Pending") {
            // For pending commissions, go to view_commission (with 30-second cancellation)
            router.push({
