// app/buddyrunner/errand_details_web.tsx
...
function ConfirmModal({
    ...
    return (
        <Modal transparent visible={visible} animationType="fade" onRequestClose={onCancel}>
            <View style={confirmModalStyles.backdrop}>
                <View style={confirmModalStyles.card}>
                    <Text style={confirmModalStyles.title}>{title}</Text>
                    <Text style={confirmModalStyles.msg}>{message}</Text>
                    <View style={confirmModalStyles.actions}>
                        <TouchableOpacity onPress={onCancel} style={confirmModalStyles.btnGhost} activeOpacity={0.9}>
                            <Text style={confirmModalStyles.btnGhostText}>Cancel</Text>
                        </TouchableOpacity>
                        <TouchableOpacity onPress={onConfirm} style={confirmModalStyles.btnSolid} activeOpacity={0.9}>
                            <Text style={confirmModalStyles.btnSolidText}>Log out</Text>
                        </TouchableOpacity>
                    </View>
                </View>
            </View>
        ...
        <Modal transparent visible={visible} animationType="fade" onRequestClose={onClose}>
            <View style={successModalStyles.backdrop}>
                <View style={successModalStyles.card}>
                    <View style={successModalStyles.iconWrap}>
                        <Ionicons name="checkmark-circle" size={44} color={colors.maroon} />
                    </View>
                    <Text style={successModalStyles.title}>{title}</Text>
                    <Text style={successModalStyles.msg}>{message}</Text>
                    <TouchableOpacity onPress={onClose} style={successModalStyles.okBtn} activeOpacity={0.9}>
...
export default function ErrandDetailsWeb() {
    ...
    const fetchData = useCallback(async () => {
        ...
        `
            ...
            // Fetch caller information (instead of runner)
            if ((e as ErrandRow).buddycaller_id) {
                const { data: p } = await supabase
                    .from("users")
                    .select("id, first_name, last_name, course, student_id_number, profile_picture_url")
                    .eq("id", (e as ErrandRow).buddycaller_id)
                    .single();

                setCaller((p ?? null) as UserRow | null);
            } else {
                setCaller(null);
            }
        } catch (err: any) {
    ...
    const callerInfo =
        (hasCaller
            ? [caller?.course, caller?.student_id_number].filter(Boolean).join(" • ")
            : "") || "No info";
    ...
    const handleMessageIconClick = async () => {
        ...
        try {
            ...
            const callerName = `${callerFirstName} ${callerLastName}`.trim() || "Caller";
    ...
    const content = () => {
        ...
        return (
            <ScrollView contentContainerStyle={ui.body}>
                {/* Caller Card (showing caller info instead of runner) */}
                <View style={s.profileCard}>
                    <View style={s.profileHeader}>
                        <View style={s.profileImageContainer}>
                            {hasCaller && caller?.profile_picture_url ? (
                                <Image 
                        ...
                        <View style={s.callerInfo}>
                            {hasCaller ? (
                                <>
                                    ...
                                    <Text style={s.callerDetails}>{callerInfo}</Text>
                                    <Text style={s.callerRole}>BuddyCaller</Text>
                                </>
                            ) : (
                                <Text style={s.noCallerText}>No BuddyCaller yet</Text>
                            )}
                        </View>
                        <TouchableOpacity
                            style={s.chatButton}
                    ...
                    <View style={s.taskDetailsContent}>
                        ...
                        <View style={{ marginTop: 16 }}>
                            ...
                            <View style={s.priceBreakdownContainer}>
                                ...
                                {/* System Fee (incl. VAT) - Only shown when there's subtotal or delivery fee */}
            ...
            <View style={{ flex: 1, flexDirection: "row", position: "relative" }}>
                ...
                />
                 ...
                 <View style={web.mainArea}>
                     <Header router={router} isSmallContent={isSmallContent} isMediumContent={isMediumContent} errandId={id} isSmallScreen={isSmallScreen} onMenuPress={() => setOpen(true)} />
                    {content()}
                </View>
            </View>
        </SafeAreaView>
...
/* ============== Small UI helpers ============== */
 function Header({
     ...
     errandId?: string | string[] | null;
     isSmallScreen?: boolean;
     onMenuPress?: () => void;
 }) {
     const handleBack = () => {
         if (errandId) {
             const idParam = Array.isArray(errandId) ? errandId[0] : errandId;
             router.push({
...
const s = StyleSheet.create({
    ...
    noCallerText: { color: colors.grayText, fontSize: 13, fontStyle: "italic" },
    chatButton: {
        width: 44,
        height: 44,
        borderRadius: 22,
