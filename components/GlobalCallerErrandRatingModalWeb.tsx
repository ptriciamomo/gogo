...
const GlobalCallerErrandRatingModalWeb: React.FC = () => {
  ...
  useEffect(() => {
    console.log('GlobalCallerErrandRatingModalWeb: Setting up subscription');
    const unsubscribe = callerErrandRatingService.subscribe((newNotification) => {
      console.log('GlobalCallerErrandRatingModalWeb: Received notification:', newNotification);
      if (newNotification) {
        setNotification(newNotification);
        setVisible(true);
        console.log('GlobalCallerErrandRatingModalWeb: Modal should be visible now');
      } else {
        setVisible(false);
        ...
        setFeedback('');
        setHoveredRating(0);
        console.log('GlobalCallerErrandRatingModalWeb: Modal hidden');
      }
    });

    return () => {
      console.log('GlobalCallerErrandRatingModalWeb: Cleaning up subscription');
      unsubscribe();
    };
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    if (!notification?.errandId || !notification?.runnerId) {
      Alert.alert('Error', 'Missing errand or runner information.');
      return;
    }
    ...
    try {
      ...
      // Save rating and feedback to database (Caller rating Runner for Errand)
      const { error: insertError } = await supabase
        .from('rate_and_feedback')
        .insert({
          errand_id: notification.errandId,
          buddycaller_id: user.id,
          buddyrunner_id: notification.runnerId,
        ...
        <View style={styles.modal}>
          ...
          <View style={styles.content}>
            <Text style={styles.subtitle}>
              How was your experience working with {notification.runnerName} on &quot;{notification.errandTitle}&quot;?
            </Text>
...
export default GlobalCallerErrandRatingModalWeb;
