...
interface UseFileUploadRunnerProps {
...
interface UseFileUploadRunnerReturn {
...
export const useFileUploadRunner = ({
  conversationId,
  user,
  currentMessage,
  setCurrentMessage,
  ...
  setMessages,
  scrollToBottom,
}: UseFileUploadRunnerProps): UseFileUploadRunnerReturn => {
  ...
  const handlePickDocument = async () => {
    try {
      ...
      if (!result.canceled && result.assets[0]) {
        ...
        // Upload document to Supabase Storage using FormData (mobile) or Blob (web)
        try {
