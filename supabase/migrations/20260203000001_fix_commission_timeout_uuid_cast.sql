-- ============================================================================
-- Migration: Fix Commission Timeout UUID/Text Type Mismatch
-- ============================================================================
-- 
-- Purpose: Fix PostgreSQL type error when reassigning commissions to next
--          runner in queue during timeout processing
-- 
-- Error: column "notified_runner_id" is of type uuid but expression is of type text
-- 
-- Root Cause: commission_ranked_runner_ids is TEXT[], and when extracting
--             runner IDs from the array, the result is TEXT. The UPDATE
--             statement fails because notified_runner_id column is UUID type.
-- 
-- Fix: Cast array element to UUID when extracting:
--      commission_ranked_runner_ids[index]::uuid
-- 
-- Scope: Commission reassignment logic ONLY
--        Errand logic unchanged (byte-for-byte)
--        Variable type changed: commission_next_runner_id TEXT → uuid
-- ============================================================================

-- Recreate the function with UUID cast fix
...
DECLARE
  ...
  commission_next_runner_id uuid;  -- FIX: Changed from TEXT to uuid
...
-- VERIFICATION
-- ============================================================================
-- Verify the function was created successfully and fix is present
DO $$
DECLARE
  func_def TEXT;
BEGIN
  ...
  -- Verify the UUID cast fix is present in commission section
  IF func_def !~* 'commission_next_runner_id.*uuid' THEN
    RAISE EXCEPTION 'Fix verification failed: commission_next_runner_id variable type not changed to uuid';
  END IF;
  
  IF func_def !~* 'commission_ranked_runner_ids\[commission_next_index \+ 1\]::uuid' THEN
    RAISE EXCEPTION 'Fix verification failed: Commission array extraction missing UUID cast';
  END IF;
  
  -- Verify errand logic is unchanged (still uses TEXT)
  IF func_def !~* 'errand_next_runner_id.*TEXT' THEN
    RAISE EXCEPTION 'Verification failed: Errand variable type was changed (should remain TEXT)';
  END IF;
  
  RAISE NOTICE '✅ Migration applied successfully: Commission UUID cast fix verified';
  RAISE NOTICE '✅ Errand logic unchanged: errand_next_runner_id remains TEXT';
END $$;
