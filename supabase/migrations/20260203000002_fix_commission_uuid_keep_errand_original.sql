-- ============================================================================
-- Migration: Commission-Only UUID Cast Fix (Errand Logic Preserved)
-- ============================================================================
-- 
-- Purpose: Apply UUID cast fix to commissions ONLY while ensuring errand
--          logic remains exactly as it was in the original implementation
-- 
-- Commission Fix: Cast TEXT array element to UUID when extracting runner ID
--                 to match notified_runner_id column type (UUID)
-- 
-- Errand Logic: Revert to original implementation (no UUID casts, all
--              RAISE statements preserved)
-- 
-- Scope: Commission reassignment logic ONLY
--        Errand logic unchanged (byte-for-byte match with original)
-- ============================================================================

-- Recreate the function with commission UUID fix and original errand logic
...
DECLARE
  ...
  commission_next_runner_id uuid;  -- FIX: Changed from TEXT to uuid for commission
...
-- VERIFICATION
-- ============================================================================
-- Verify the function was created successfully and only commission logic differs
DO $$
DECLARE
  func_def TEXT;
BEGIN
  ...
  -- Verify commission UUID cast fix is present
