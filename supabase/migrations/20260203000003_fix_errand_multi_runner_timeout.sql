-- ============================================================================
-- Migration: Fix Errand Multi-Runner Timeout Bug
-- ============================================================================
-- 
-- Purpose: Fix queue advancement bug in process_timed_out_tasks() that prevents
--          multi-runner errands from advancing to the next runner on timeout
-- 
-- Bug: The errand reassignment UPDATE WHERE clause was missing an
...
-- 
-- Fix: Add `AND notified_runner_id = errand_rec.notified_runner_id` to
--      the WHERE clause in the errand reassignment UPDATE
-- 
...
-- Recreate the function with errand idempotency guard fix
...
-- VERIFICATION
-- ============================================================================
-- Verify the function was created successfully and errand idempotency guard is present
DO $$
DECLARE
  func_def TEXT;
BEGIN
  ...
  -- Verify errand reassignment UPDATE has idempotency guard
  IF func_def !~* 'AND notified_runner_id = errand_rec\.notified_runner_id' THEN
    RAISE EXCEPTION 'Fix verification failed: Errand reassignment UPDATE missing idempotency guard';
  END IF;
  
  RAISE NOTICE '✅ Migration applied successfully: Errand multi-runner timeout fix verified';
END $$;
