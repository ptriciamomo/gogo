-- ============================================================================
-- Migration: Fix Errand Timeout UUID Cast (Errands ONLY)
-- ============================================================================
--
-- Purpose:
--   Fix production crash in process_timed_out_tasks() during ERRAND reassignment:
--     "column \"notified_runner_id\" is of type uuid but expression is of type text"
--
-- Root cause:
--   errand.ranked_runner_ids is TEXT[] and errand_next_runner_id is TEXT, but
--   errand.notified_runner_id is UUID. The errand reassignment UPDATE was
--   assigning TEXT into UUID without an explicit cast.
--
-- Scope / Constraints:
--   ✅ Errands only: apply cast only in the errand reassignment UPDATE
--   ❌ Do NOT modify commissions in any way (logic must remain unchanged)
--   ❌ Do NOT modify loops, queue logic, timeout duration, or cancel paths
--   ❌ Do NOT modify Edge Functions
--
-- Change:
--   In errand reassignment UPDATE:
--     notified_runner_id = errand_next_runner_id::uuid
--
-- ============================================================================
    ...
    BEGIN
      ...
      ELSE
        ...
        WHILE errand_check_index < errand_queue_length AND errand_next_runner_id IS NULL LOOP
          -- Skip invalid UUID strings to prevent cast crashes
          IF errand_ranked_runner_ids[errand_check_index + 1] IS NULL OR 
             errand_ranked_runner_ids[errand_check_index + 1] !~* '^[0-9a-f-]{36}$' THEN
            -- Invalid UUID format, skip and advance
            RAISE NOTICE '[TIMEOUT] Errand %: Skipping invalid UUID at index %: %', 
              errand_rec.id, errand_check_index, errand_ranked_runner_ids[errand_check_index + 1];
            errand_check_index := errand_check_index + 1;
          -- Check if runner at this index is already timed out (only if valid UUID)
          ELSIF errand_ranked_runner_ids[errand_check_index + 1]::uuid = ANY(errand_updated_timeout_ids) THEN
            ...
            errand_check_index := errand_check_index + 1;
          ELSE
            -- Runner is not timed out, select them (valid UUID confirmed by pattern check above)
        ...
        ELSE
          ...
          UPDATE errand
          SET 
            notified_runner_id = errand_next_runner_id::uuid,  -- FIX: Cast TEXT to UUID for errand.notified_runner_id
            notified_at = NOW(),
